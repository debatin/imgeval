...
#define SAVEIMG_1D 0x40   ///<the side curves should be saved
#define SAVEIMG_BLOC 0x80   ///<raw data as used for error pattern fitting
#define SAVEIMG_MAT 0x40   ///<the side curves should be saved
...
class Atomimg: public QWidget
{
    ...
    void perform1DFit(vector<float> fitdaty, vector<float> &fitparams,string direction,  GraphPlotter* graph);
    vector<float> perform1DSpecialBimodal(vector<float> fitdaty, vector<float> fitdatx,  GraphPlotter* graph);
    ...
    void storeFitparams(const vector<float>&fitparams,  //< stores the fitted variables in a generic way
                       string pname);
void search_errorpatterns();  //< searches the mein imgstack for errorpatterns
void errPatternFit();   //< fits errorpatterns
void savePNG();
    void saveMAT();
    void saveBLOC();
...
public slots:
 ...
 private:
    ...
    map<int,Imagedata> errorpatterns; ///< containing possible error patterns
    int nerrpat;
    Imagedata errmatrix;
    ...
    ImageMarker* marker1;    ///< various coloured marker
    ImageMarker* marker2;    ///< various coloured marker
    ImageMarker* marker3;    ///< various coloured marker
    ImageMarker* marker4;    ///< various coloured marker
    ImageMarker* fitstart_marker;///< parameter to set fit start param
    std::vector<ImageMarker*> marker;
    ROI* roi_fit;             ///< Region of interest for the fits
    ROI* roi_util;             ///< 1st utility ROI
    ROI* roi_util2;             ///< 2nd utility ROI
...
public:
    MainWindow* mainapp;
private:
    int showimage; //0 data 1 imgatoms 2 noatoms 3 Background 4 backgroundn 5 imgstack s.u.
    string showimagestr; //name of image in imgstack
};
