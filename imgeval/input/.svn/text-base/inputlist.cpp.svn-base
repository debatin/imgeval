    ...
    :QWidget(parent)
{
    ...
    //Button etc. for the Measurement Part
...
/*! called if different dataset is selected
\author Markus Debatin
\date 10-2010
*************************************************************************/
...
void InputList::readUdpDatagram()
{
    ...
    while (udp->hasPendingDatagrams()) {
        ...
        udp->readDatagram(data.data(), data.size(),
                          &sender, &senderPort);
        string s = data.data();
        if(s.rfind("\\",15) != string::npos)    /*if \ found (you have to escape \ thats why its \\) */
            thread->addUdp(s.substr(s.rfind("\\")+1,4));
        if(s.rfind("Rbdetuning") != string::npos)   {
            std::string dnums = s.substr(s.rfind("Rbdetuning")+10,4);
            double dnum = ::atof(dnums.c_str());
            settings->store("Rbdetuning",dnum,'p');
        }
        if(s.rfind("Csdetuning") != string::npos)   {
            std::string dnums = s.substr(s.rfind("Csdetuning")+10,4);
            double dnum = ::atof(dnums.c_str());
            settings->store("Csdetuning",dnum,'p');
        }
    }
}
